...
void Shop::killed() {
  ...
  if (check.getName(shopLvl) == "Malacoda" && enemyTracker == 0) {
    hasBeenKilled = true;
  } else if (check.getName(shopLvl) == "Behemoth King" && enemyTracker == 1) {
    hasBeenKilled = true;
  } else if (check.getName(shopLvl) == "Ironclad Golem" && enemyTracker == 2) {
    hasBeenKilled = true;
  } else if (check.getName(shopLvl) == "Shadow Dragon" && enemyTracker == 3) {
    hasBeenKilled = true;
  } else if (check.getName(shopLvl) == "Soul Devourer" && enemyTracker == 4) {
    hasBeenKilled = true;
  } else if (check.getName(shopLvl) == "Chimera" && enemyTracker == 5) {
    hasBeenKilled = true;
  } else if (check.getName(shopLvl) == "Death Knight" && enemyTracker == 6) {
    hasBeenKilled = true;
  } else if (check.getName(shopLvl) == "Thunderbird" && enemyTracker == 7) {
    hasBeenKilled = true;
  } else if (check.getName(shopLvl) == "Giant Scorpion" && enemyTracker == 8) {
    hasBeenKilled = true;
  } else if (check.getName(shopLvl) == "Bloodsucker" && enemyTracker == 9) {
    hasBeenKilled = true;
  }
}

// function to set the cash value
void Shop::setCash(int c) { cash = c; }

// function to get the cash value
void Shop::getCash() { cout << "\nYou have $" << cash << ".\n" << endl; }

// function to add cash to current cash value
void Shop::addCash(int add) { cash = cash + add; }

// function to add a potion to the current value
void Shop::addPotion() { potions++; }

// function to remove a potion from the current value
void Shop::removePotion() { potions--; }
...
void Shop::loadShop(int lvl, int c, int sword, int armor) {
  ...
  while (shopLvl == 0) {
    ...
    if ((buy == "U" || buy == "u")) {
      ...
      if (hasBeenKilled == true) {
        ...
        cout << "\nYou have to kill the Level " << shopLvl
             << " boss before you can purchase this Level Upgrade." << endl;
      }
      // if U is not entered
    } else {
  ...
  // ALL SHOPS ARE THE SAME WITH SLIGHTLY DIFFERENT NUMBERS AND PRICES, SAME UI
  // JUST LEVELS UP EACH TIME THE SHOPLVL IS INCREASED.
  ...
  while (shopLvl == 1) {
    ...
    if (buy == "U" || buy == "u" || buy == "P" || buy == "p" || buy == "S" ||
        buy == "s" || buy == "A" || buy == "a") {
      ...
      if (buy == "P" || buy == "p") {
        if (cash < 5) {
          ...
          addPotion();
          cout << "\nYou acquired a Health Potion! You now have " << potions
               << " potions." << endl;
          cash = cash - 5;
          cout << "\nCash: $" << cash << endl;
        }
  ...
  while (shopLvl == 9) {
    ...
    if (buy == "E" || buy == "e" || buy == "P" || buy == "p" || buy == "S" ||
        buy == "s" || buy == "A" || buy == "a") {
